...
public class InventoryCLI {
	...
	public void start() {
		...
		while (true) {
			System.out.println("1:商品一覧　2:商品追加　3:在庫の追加・削減　4:キーワード検索　5:商品名変更　6:商品削除　7:在庫切れの商品一覧　0:終了");
	...
	private void changeStock() {
		...
		while (true) {
			...
			if (confirm("在庫の増加を行う場合はyを、在庫の削減を行う場合はnを入力してください")) {
				...
				if (amountAdd > 0) {
					if (repo.findProduct(name).addStock(amountAdd)) {
			...
			if (amountRemove > 0) {
				if (repo.findProduct(name).removeStock(amountRemove)) {
